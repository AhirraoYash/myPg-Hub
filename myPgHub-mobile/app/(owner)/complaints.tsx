...
function ComplaintCard({ complaint, onUpdateStatus }: { complaint: Complaint; onUpdateStatus: (id: string, status: string) => void }) {
  const urgencyStyle = URGENCY_COLORS[complaint.urgency] ?? 'bg-gray-100 text-gray-700';
  const statusStyle = STATUS_COLORS[complaint.status] ?? 'bg-gray-100 text-gray-700';
  const name = complaint.tenant
    ? `${complaint.tenant.first_name} ${complaint.tenant.last_name}`
    : 'Tenant';
  const date = new Date(complaint.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' });
    ...
    <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100"
      ...
      <View className="flex-row justify-between items-start mb-2">
        <View className="flex-1 mr-2">
          ...
          <Text className="text-xs text-gray-500 mt-0.5">{name} · {date}</Text>
        </View>
        <View className="items-end gap-1.5">
          <View className={`px-2 py-0.5 rounded-md ${urgencyStyle.split(' ')[0]}`}>
            <Text className={`text-[10px] font-black ${urgencyStyle.split(' ')[1]}`}>{complaint.urgency}</Text>
          </View>
          <View className={`px-2 py-0.5 rounded-md ${statusStyle.split(' ')[0]}`}>
            <Text className={`text-[10px] font-black ${statusStyle.split(' ')[1]}`}>{complaint.status.replace('_', ' ')}</Text>
          </View>
        </View>
      </View>
      ...
      <View className="flex-row gap-2">
        ...
        {complaint.status === 'Open' && (
          <TouchableOpacity
            onPress={() => onUpdateStatus(complaint._id, 'In_Progress')}
            className="bg-blue-600 px-2.5 py-1 rounded-lg"
          >
            <Text className="text-[10px] font-black text-white">Mark In Progress</Text>
          </TouchableOpacity>
        )}
        {complaint.status === 'In_Progress' && (
          <TouchableOpacity
            onPress={() => onUpdateStatus(complaint._id, 'Resolved')}
            className="bg-emerald-600 px-2.5 py-1 rounded-lg"
          >
            <Text className="text-[10px] font-black text-white">Mark Resolved</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
    ...
    <SafeAreaView className="flex-1 bg-surface" edges={['top']}>
      <View className="px-6 pt-4 pb-3 flex-row items-center gap-4">
        ...
        <View className="flex-1">
          ...
          <Text className="text-xs text-gray-400 font-semibold">{complaints.length} total issues</Text>
        </View>
      </View>
      ...
      <View className="mx-6 mb-4 bg-gray-100 rounded-xl p-1 flex-row">
        {(['Open', 'In_Progress', 'Resolved'] as const).map(t => (
          ...
          >
            <Text className={`text-xs font-black ${activeTab === t ? 'text-blue-600' : 'text-gray-500'}`}>
              {t.replace('_', ' ')}
            </Text>
          </TouchableOpacity>
        ))}
              ...
              {filtered.length === 0
                ? (
                  <View className="items-center py-16">
                    ...
                    <Text className="text-gray-400 font-bold mt-4">No {activeTab.replace('_', ' ')} complaints</Text>
                  </View>
                )
                : filtered.map(c => (
                  <ComplaintCard key={c._id} complaint={c} onUpdateStatus={handleUpdateStatus} />
                ))
              }
            </View>
          </ScrollView>
