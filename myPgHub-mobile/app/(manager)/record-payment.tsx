...
export default function ManagerRecordPaymentScreen() {
  ...
  const [form, setForm] = useState({
    amount: '', method: 'Cash' as PaymentMethod, transaction_id: '', notes: '',
  });
  ...
  const handleSubmit = async () => {
    if (!selectedInvoice) { Alert.alert('Select Invoice', 'Please select an invoice.'); return; }
    if (!form.amount || Number(form.amount) <= 0) { Alert.alert('Invalid Amount', 'Enter a valid amount.'); return; }
    setSubmitting(true);
    try {
      await recordPayment({
      ...
      Alert.alert('Recorded ✓', 'Payment has been recorded.', [{ text: 'Done', onPress: () => router.back() }]);
    } catch (err: any) {
          ...
          <View className="pt-2 pb-12 gap-4">
            ...
            <View className="bg-white rounded-2xl p-4 border border-gray-100"
              ...
              {loadingInvoices
                ...
                : invoices.length === 0
                  ? <Text className="text-sm text-gray-400 text-center py-4">No unpaid invoices</Text>
                  : invoices.map(inv => {
                    ...
                    return (
                      <TouchableOpacity key={inv._id}
                        onPress={() => { setSelectedInvoice(inv); setForm(f => ({ ...f, amount: String(inv.amount_due) })); }}
                        ...
                        <Text className={`text-base font-black ${isSelected ? 'text-blue-700' : 'text-gray-900'}`}>
            ...
            <View className="bg-white rounded-2xl p-4 border border-gray-100 gap-4"
              ...
              <View>
                <Text className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-2 ml-1">Method</Text>
                <View className="flex-row gap-2">
                  {PAYMENT_METHODS.map(m => (
                    <TouchableOpacity key={m} onPress={() => setForm(f => ({ ...f, method: m }))}
                      className={`flex-1 py-3 rounded-xl border items-center ${form.method === m ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-200'}`}
                      activeOpacity={0.7}>
                      <Text className={`text-xs font-black ${form.method === m ? 'text-white' : 'text-gray-700'}`}>{m.replace('_', ' ')}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
